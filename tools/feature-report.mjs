...
//
// 仮想ユーザーは診断14項目のうち4項目を無作為に「重視する項目」として持つので、
// どの項目も本当は 4/14 = 28.6% の人にとって重要。
...
//
// 出やすさが公平な割合（上位3つで 3/14 = 21.4%）より大きく、
// かつ確からしさが低い項目は「出やすいだけ」。
...
for (let t = 0; t < TRIALS; t++) {
  ...
  // 落ちて、同じ種でも毎回ちがう結果になる。プールの前後を比べられなくなるので、
  // ここで種を渡しておく（アプリ側は毎回ちがってよいので既定のままでよい）。
...
const BASE = 3 / KEYS.length; // どの項目も、本当は 3/14 = 21.4% の割合で上位3つに入るはず
...
console.log('');
console.log(`かたより = 上位3つに出る割合 ÷ ${pct(BASE)}。1.0 なら公平、大きいほど出やすい項目。`);
